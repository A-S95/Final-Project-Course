...
function LedgerCard() {
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-sm rounded-2xl border border-border bg-surface-raised p-5 shadow-[0_30px_60px_-20px_rgba(101,82,245,0.35)]"
    >
      <div className="flex items-center justify-between border-b border-border pb-3">
        <div>
          <p className="text-xs text-ink-subtle">Agosto 2026</p>
          <p className="font-display text-lg font-semibold text-ink">1 960,28 €</p>
        </div>
    ...
    <section className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-12 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:py-24">
      <div className="flex flex-col gap-6">
        ...
        >
          O teu dinheiro,
          <br />
          <span className="brand-gradient bg-clip-text text-transparent">à vista.</span>
        </motion.h1>
        <motion.p {...fadeUp(0.16)} className="max-w-md text-lg text-ink-muted">
          Contas, orçamentos e objetivos num só sítio — sozinho ou a dois, sem folhas de cálculo
          e sem surpresas no fim do mês.
        </motion.p>
        <motion.div {...fadeUp(0.24)} className="flex flex-wrap items-center gap-3">
          <Link to="/registar" className={buttonVariants({ size: 'lg' })}>
            Criar conta grátis
          </Link>
...
const FEATURES: Feature[] = [
  {
    ...
    description:
      'Saldo, receitas e despesas do mês num relance, com alertas automáticos quando algo foge do padrão.',
  },
  {
    title: 'Orçamentos por categoria',
    description:
      'Define um limite mensal por categoria e vê a barra de progresso encher-se — sabes antes de estourar o orçamento, não depois.',
  },
  {
    title: 'Objetivos com prazo',
    description:
      'Poupança direcionada a um objetivo concreto, com o progresso e o prazo sempre visíveis.',
  },
]
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-2xl border border-border bg-surface-raised p-6">
      ...
      <motion.div
        ...
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, margin: '-80px' }}
        transition={{ duration: 0.4, delay: reduceMotion ? 0 : 1 }}
      ...
      <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2">
        <div className="flex flex-col gap-4">
          ...
          <p className="text-lg text-ink-muted">
            Cada pessoa mantém a sua própria conta e categorias — mas quando a renda, a luz ou a
            água são despesas da casa, marca-as como partilhadas. O agregado familiar conta-as
            uma vez, nunca duas, mesmo que cada um tenha a sua própria categoria "Renda".
          </p>
          <Link
            to="/registar"
    ...
    <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:py-24">
      ...
      >
        <h2 className="font-display max-w-xl text-3xl font-semibold tracking-tight text-ink sm:text-4xl">
          Começa a organizar as tuas finanças hoje.
        </h2>
        <Link to="/registar" className={buttonVariants({ size: 'lg' })}>
          Criar conta grátis
        </Link>
