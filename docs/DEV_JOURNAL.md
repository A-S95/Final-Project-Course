...
**Causa 1 — uma falha temporária do backend matava a sessão.** O `doRefresh()` fazia `if (!response.ok) { setAccessToken(null); return null }`, ou seja, tratava **qualquer** resposta não-OK como "refresh token rejeitado". Mas `!response.ok` apanha tanto um 401 verdadeiro como um 502/503 transitório — e o backend está na Render no plano grátis, que **adormece ao fim de 15 minutos** (a mesma janela do `access_token_expire_minutes = 15`). Cenário: estou parado no formulário 15+ min, o access token expira **e** a Render adormece ao mesmo tempo; a próxima query leva 401, o `refreshSession()` acorda a Render mas o primeiro pedido leva 502 → `doRefresh` devolve `null` → o `authedFetch` não faz retry → a query rebenta com 401. Como o `AuthProvider` não é reavaliado, `status` continua `'authenticated'` e fico preso na página com os erros. Segundos depois a Render está acordada e o F5 funciona — daí o "às vezes".

**Causa 2 — corrida na rotação de refresh tokens (mais rara, pior).** O backend faz rotação com deteção de roubo: reutilizar um token já rodado revoga **a família toda** (`auth_service.revoke_all_for_user`). O `inFlightRefresh` que introduzi na Fase 13 só resolve isto **dentro de um contexto de página** — não cobre a **PWA instalada + uma aba do browser abertas ao mesmo tempo** (dois contextos JS, cookie partilhado), nem um F5 durante um pedido lento (o pedido da página antiga ainda chega ao servidor e o novo load manda outro com o mesmo cookie). Dois refreshes com o mesmo cookie → o segundo aciona a deteção de roubo → sessão morta em todo o lado até novo login.

**Correções (cinco, decididas com o Claude a explicar os trade-offs):**

1. **`doRefresh` distingue "sessão terminada" de "tenta outra vez".** Só 401/403 → sessão mesmo terminada (`setAccessToken(null)` + avisa o `AuthContext`). Erro de rede, 5xx ou 409 → lança `RefreshError`, que o chamador pode repetir sem deslogar.
2. **`QueryClient` com `retry` inteligente** (`frontend/src/main.tsx`): repete falhas transitórias (útil para a Render a acordar), mas **nunca** 401/403/404. Mais `staleTime: 30s`.
3. **`AuthProvider` com `.catch()` no arranque**: uma falha de rede ao verificar a sessão já não deixa a app presa no splash para sempre nem desloga — mostra um ecrã **"Sem ligação ao servidor"** com botão *Tentar novamente* (`components/splash.tsx`, novo estado `'connection-error'` distinto de `'unauthenticated'`). Também: quando o refresh token é mesmo rejeitado a meio do uso, o cliente HTTP avisa o `AuthContext` (`setSessionExpiredHandler` em `token-store.ts`) e a app passa a mandar para o login em vez de deixar erros inline numa página protegida.
4. **Janela de tolerância na rotação (backend).** Nova coluna `refresh_tokens.revoked_at` (migração `d1f4a2c6b8e3`) + `refresh_reuse_grace_seconds = 10`. Reutilizar um token rodado **há menos de 10s** deixou de ser tratado como roubo: o backend responde **409** (nova exceção `RefreshTokenRaceError`), **sem limpar o cookie e sem revogar nada** — o pedido paralelo legítimo já rodou para um cookie novo, que o cliente relê e usa. Reutilização **depois** dos 10s continua a revogar a família toda (o caso realista de roubo: token exfiltrado e usado mais tarde). Trade-off aceite e fácil de defender: um token roubado e usado nos primeiros 10 segundos não dispara o alarme — em troca, os falsos positivos benignos (PWA+aba, F5, retry) deixam de expulsar o utilizador. É o que a RFC 9700 (OAuth 2.0 Security BCP) sugere para rotação de refresh tokens.
5. **Lock entre abas com `navigator.locks`** (`refreshSession` em `client.ts`): só um contexto da mesma origem chega ao `/refresh` de cada vez, mesmo com a PWA e uma aba abertas em simultâneo. Degrada para chamada direta onde a API não existe. Isto sozinho já elimina o caso PWA+aba antes de chegar ao backend; a correção 4 é a rede de segurança para o F5-durante-pedido-lento e browsers sem a API.

**Testes.** Backend: `test_reusing_a_rotated_refresh_token_revokes_the_whole_family` passou a `..._outside_the_grace_window_...` — com `monkeypatch.setattr(settings, "refresh_reuse_grace_seconds", 0)` qualquer reutilização cai no caminho de roubo (mais determinístico do que backdatar o `revoked_at` na base de dados através de uma sessão partilhada entre threads pelo `TestClient`, que se revelou não fiável). Novo `test_immediate_refresh_token_reuse_is_treated_as_a_benign_race` (409 + a sessão legítima continua a 200); `test_refresh_rotates_the_refresh_token` passou a esperar 409 na reutilização imediata. `ruff` limpo, 23 testes unitários a passar. Frontend: `tsc`/`oxlint`/`build` limpos. **Por correr nesta máquina (Docker Desktop estava desligado):** a migração Alembic, a suite `pytest` completa e os E2E — falta correr `docker compose up -d`, `uv run alembic upgrade head`, `uv run pytest -q` e `npm run test:e2e` antes do commit.

**Para a apresentação:** bom exemplo de um bug que só aparece em produção (Render a adormecer) e sob concorrência real entre contextos (PWA + browser) — o `inFlightRefresh` da Fase 13 resolvia a corrida *dentro* de uma página, mas o problema real era *entre* páginas/abas. A solução combina uma defesa no cliente (lock entre abas) com uma no servidor (janela de tolerância), e distingue três coisas que o código antes confundia num só `!response.ok`: sessão terminada, servidor indisponível, e corrida benigna.
