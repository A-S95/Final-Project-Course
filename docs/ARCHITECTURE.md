...
- Refresh token: duração longa (30 dias), token opaco de alta entropia (`secrets.token_urlsafe`, não um JWT), guardado em **cookie `httpOnly`, `Secure` (fora de `development`)**, com `Path=/api/v1/auth` — só é enviado nos pedidos de auth, nunca nos outros pedidos à API. Persistido como hash **SHA-256** (não bcrypt — é um segredo aleatório de 512 bits, não uma password humana; não há força bruta a mitigar, só um leak da tabela a evitar) na tabela `refresh_tokens`, com rotação a cada `/refresh` (o token antigo é sempre revogado, mesmo em caso de reutilização indevida) para permitir revogação/logout real.
  - **Deteção de reutilização + janela de tolerância.** Reutilizar um token já rodado normalmente indica roubo, e revoga **toda a família** de tokens do utilizador. Exceção: se o token foi rodado **há menos de `refresh_reuse_grace_seconds` (10s)**, trata-se como uma **corrida benigna** — o mesmo cookie enviado duas vezes quase em simultâneo (PWA instalada + aba do browser, F5 durante um pedido lento, retry de rede) — e o backend responde `409` sem revogar nada. Reutilização depois da janela continua a revogar a família. Segue a RFC 9700 (OAuth 2.0 Security BCP). O cliente serializa ainda os refreshes entre abas com `navigator.locks`, e um único pedido em curso por contexto (`inFlightRefresh`). Ver `DEV_JOURNAL.md`, entrada "Bug real: 'Não autenticado' às vezes ao criar cartões/contas". Coluna `refresh_tokens.revoked_at` guarda quando cada token foi revogado.
...
- **Recuperação de password** (2026-08-31): endpoints `POST /auth/password-reset/request` (responde sempre `202` com a mesma mensagem — não revela se o email existe) e `POST /auth/password-reset/confirm`. Token opaco de alta entropia, hash **SHA-256** na tabela `password_reset_tokens`, prazo de 1h, **uso único** (`used_at`), e um novo pedido invalida os anteriores. Repor a password **revoga todos os refresh tokens** do utilizador. Email via **Resend** (API HTTP, `urllib` da stdlib — sem dependência nova) com fallback para escrever o link no log quando não há `RESEND_API_KEY` (dev/testes funcionam sem configurar nada).
- **Cabeçalhos de segurança HTTP** (2026-08-31): o backend (`app/core/security_headers.py`, middleware) devolve `X-Content-Type-Options`, `X-Frame-Options: DENY`, `Referrer-Policy`, `Cross-Origin-Opener-Policy`, `Permissions-Policy` e uma CSP mínima (`frame-ancestors 'none'`) em todas as respostas; `Strict-Transport-Security` só em produção. O frontend tem a CSP a sério em `frontend/vercel.json` (`script-src 'self'` — sem nenhum script inline — `style-src` com `'unsafe-inline'` para o `motion`/Recharts, `connect-src` limitado à API), fonte única lida também pelo `vite.config.ts` para as violações aparecerem já em `npm run dev`.
...
**Erros de domínio → HTTP num handler central** (desde 2026-08-31): todas as exceções de negócio herdam de `DomainError` (`core/exceptions.py`), que carrega um `status_code` e um `detail`. Um único `@app.exception_handler(DomainError)` em `main.py` traduz qualquer uma numa resposta JSON — os routers não apanham nada, só chamam o serviço e fazem `db.commit()`. Antes, cada endpoint repetia `try/except ... raise HTTPException(...)` (58 blocos no total). Exceção: `/auth/refresh` mantém o `try/except` explícito porque a resposta de erro aí também mexe no cookie e faz um commit extra.
...
**Segurança**:
- ~~Recuperação de password~~ — **feito** (2026-08-31, ver `DEV_JOURNAL.md`): token de reset SHA-256 com prazo de 1h e uso único, endpoints `/auth/password-reset/request|confirm`, envio via Resend com fallback para consola em dev. Falta ainda a **confirmação de email no registo** (campo `email_verified` + token de verificação) — agora que o envio de email existe, é mais pequeno.
- ~~Cabeçalhos de segurança HTTP~~ — **feito** (2026-08-31): middleware no backend (`app/core/security_headers.py`) + CSP no frontend (`frontend/vercel.json`, fonte única lida também pelo `vite.config.ts`).
...
- Monitorização de erros em produção (Sentry, tier grátis) — hoje os erros só ficam nos logs estruturados da Render.

**Funcionalidade**:
- ~~Exportar transações em CSV~~ — **feito** (2026-08-31): `GET /transactions/export`, mesmos filtros da lista, separador `;` + vírgula decimal + BOM (Excel PT). Falta PDF do resumo mensal, se fizer sentido.
...
**Recibos — armazenamento a mudar**: hoje em disco local (`backend/uploads/receipts/`), que na Render grátis não persiste entre reinícios. Decisão tomada (2026-08-31, por implementar): mover para uma coluna `bytea` no Postgres, baixando o limite de 8MB para ~2MB. Alternativa considerada e adiada: Cloudflare R2 (S3).
